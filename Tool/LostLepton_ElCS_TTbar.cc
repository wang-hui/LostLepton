...
#include "LostLepton_ElCS_TTbar.h"
#include "TGraph.h"
#include "TCanvas.h"
#include "TH1F.h"
#include "TH1D.h"
...
int main(int argc, char* argv[])
{
  ...
  if (argc < 2)
  {
    ...
    std::cerr <<" ./LostLepton_ElCS_TTbar runlist_ttjets.txt isoplots.root" << std::endl;
    return -1;
  }
  ...
  while(trCS.getNextEvent())
  {
	...
	vector<TLorentzVector> elesLVec = trCS.getVec<TLorentzVector>("elesLVec");
          ...
          if( fabs( elesLVec[ie].Eta()) < 2.5 && elesRelIso[ie] < 0.24 )
	  {
        ...
        double deltaphi_els = DeltaPhi( reco_els_phi , metphi );
        double mtW_els = std::sqrt( 2.0 * reco_els_pt * met * (1.0 - cos(deltaphi_els) ) );
        ...
        if ( mtW_els < 100.0 )
	{
	  ...
	  (myBaseHistgram.h_pred_el_all_topmass)->Fill(bestTopJetMass, EventWeight_all_els*EventWeight_els);
        ...
        }//mtW5_muss<100 (electron CS)
      }//nElectrons == 0 && nMuons == 1 (electron CS)
    }//baseline_nolepveto
  }
...
void AccRecoIsoEffs::EffstoWeights()
{
  ...
  for(i_cal = 0 ; i_cal < PT_BINS ; i_cal++)
  {
    ...
    mus_EventWeight_iso[i_cal]  = 1.0/els_isoeff[i_cal]/els_recoeff[i_cal]*(1.0 - mus_acc)/els_acc;
    mus_EventWeight_reco[i_cal] = 1.0/els_isoeff[i_cal]*(1 - mus_recoeff[i_cal])/els_recoeff[i_cal]*mus_acc/els_acc; 
    mus_EventWeight_acc[i_cal]  = (1.0 - mus_isoeff[i_cal])/els_isoeff[i_cal]*mus_recoeff[i_cal]/els_recoeff[i_cal]*mus_acc/els_acc;
  }
