{
  TFile f_eff("TagAndProbe2dPlots.root");

  TCanvas *c1 = new TCanvas("c1", "c1",0,51,1920,1004);
  c1->SetFillColor(0);
  c1->cd();

  TH2F *h_mu_iso_eff_ori = (TH2F*)f_eff.Get("mus_isoeffs_tagandprobe");
  ...
  TH2F *h_mus_nreco_ori = (TH2F*)f_eff.Get("mus_nreco_tagandprobe");
  //h_els_iso_eff_ori->Draw("colztext");

  TH2F *h_mus_nreco;
  //Float_t xbins[9]={5.0,10.0,20.0,30.0,40.0,50.0,70.0,100.0,120.0};
  //Float_t ybins[9]={0.0,5.0,10.0,20.0,40.0,60.0,80.0,100.0,120.0};
  h_mus_nreco = new TH2F("h_mus_nreco", "h_mus_nreco", 8, xbins, 8, ybins);
  h_mus_nreco->SetTitle("");
  h_mus_nreco->SetXTitle("muon pT [GeV]");
